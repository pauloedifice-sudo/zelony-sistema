-- Complementa a correcao da ativacao do legado tratando vendas antigas
-- que guardam a data comercial somente em DD/MM.
-- Regra:
-- 1. Se houver venda ativa anterior a 2026-07-01, usa a primeira data comercial.
-- 2. Se nao houver venda ativa anterior a 2026-07-01, fixa 2026-07-01.
-- 3. Corrige junto o evento de backfill no historico_status.

with parametros as (
  select
    date '2026-07-01' as data_base,
    2026::int as ano_base
),
usuarios_legado as (
  select
    u.id,
  ...
  select
    ...
    coalesce(
      ...
      case
        ...
        when trim(coalesce(v.data, '')) ~ '^\d{2}/\d{2}$' then to_date(trim(v.data) || '/' || p.ano_base::text, 'DD/MM/YYYY')
        else null
      end
    ) as data_venda
  ...
  left join lateral (
    select
      case
        ...
        when trim(coalesce(h.elem->>'d', '')) ~ '^\d{2}/\d{2}$' then to_date((h.elem->>'d') || '/' || p.ano_base::text, 'DD/MM/YYYY')
...
set
  data_ativacao = ac.data_ativacao_corrigida,
  historico_status = coalesce(hc.historico_status_corrigido, u.historico_status)
from ativacao_corrigida ac
left join historico_corrigido hc on hc.id = ac.id
where u.id = ac.id
  and ac.data_ativacao_corrigida is not null
  and (
    u.data_ativacao is distinct from ac.data_ativacao_corrigida
    or (
      hc.historico_status_corrigido is not null
