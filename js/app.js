...
function showVDetail(id){
  ...
  let mH='';
  if(role==='cor'){
    mH=`<div class="mc a"><div class="mc-l">${zUiText('Valor de venda')}</div><div class="mc-v g">${fmt(v.valor)}</div></div><div class="mc"><div class="mc-l">${zUiText('Minha comissÃ£o')}</div><div class="mc-v g">${fmt(comC(v))}</div><div class="mc-s">${(v.pct_cor*100).toFixed(1)}% ${zUiText('lÃ­q.')}</div></div><div class="mc"><div class="mc-l">${zUiText('Etapa')}</div><div class="mc-v">${v.etapa+1}/${ETAPAS.length}</div></div><div class="mc"><div class="mc-l">${zUiText('Status')}</div><div class="mc-v" style="font-size:13px;">${isFinal?zUiText('✓ Pago'):zUiText('Andamento')}</div></div>`;
  }else if(role==='cap'){
