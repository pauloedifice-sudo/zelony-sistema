...
function showVDetail(id){
  ...
  if(role==='cor'){
    ...
    mH=`<div class="mc a"><div class="mc-l">${zUiText('Valor venda')}</div><div class="mc-v g">${fmt(v.valor)}</div></div><div class="mc"><div class="mc-l">${zUiText('Com. bruta')}</div><div class="mc-v">${fmt(comBruta(v))}</div><div class="mc-s">${pctSeguro(v.pct,0)}</div></div><div class="mc"><div class="mc-l">${zUiText('Com. lÃ­quida')}</div><div class="mc-v g">${fmt(comTotal(v))}</div></div><div class="mc"><div class="mc-l">${zUiText('Zelony')}</div><div class="mc-v">${fmt(comZ(v))}</div></div>`;
  }
  ...
  const impostoRow=['dir','fin'].includes(role)?`<div style="margin-top:8px;border-top:1px dashed rgba(184,144,42,0.25);padding-top:8px;"><div class="eq-row" style="background:#FEF6EC;border-radius:6px;padding:6px 8px;margin:0 -2px;"><div style="width:25px;height:25px;border-radius:50%;background:#FEF0DC;border:1px solid #E8A040;display:flex;align-items:center;justify-content:center;font-size:11px;flex-shrink:0;">${zUiText('📋')}</div><div style="flex:1"><div style="font-size:12px;font-weight:500;color:#A05010;">${zUiText('Imposto sobre comissão')}</div><div style="font-size:9px;color:#B07030;">${pctSeguro(v.imp,0)} ${zUiText('sobre comissão bruta')} (${fmt(comBruta(v))})</div></div><span style="font-size:13px;font-weight:600;color:#C06020;">- ${fmt(valorImposto)}</span></div></div>`:'';
