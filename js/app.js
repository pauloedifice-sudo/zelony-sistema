...
function showVDetail(id){
  ...
  let mH='';
  if(role==='cor'){
    mH=`<div class="mc a"><div class="mc-l">${zUiText('Valor de venda')}</div><div class="mc-v g">${fmt(v.valor)}</div></div><div class="mc"><div class="mc-l">${zUiText('Minha comissÃ£o')}</div><div class="mc-v g">${fmt(comC(v))}</div><div class="mc-s">${(v.pct_cor*100).toFixed(1)}% ${zUiText('lÃ­q.')}</div></div><div class="mc"><div class="mc-l">${zUiText('Etapa')}</div><div class="mc-v">${v.etapa+1}/${ETAPAS.length}</div></div><div class="mc"><div class="mc-l">${zUiText('Status')}</div><div class="mc-v" style="font-size:13px;">${isFinal?zUiText('âœ“ Pago'):zUiText('Andamento')}</div></div>`;
  }else if(role==='cap'){
    ...
    mH=`<div class="mc a"><div class="mc-l">${zUiText('Valor venda')}</div><div class="mc-v g">${fmt(v.valor)}</div></div><div class="mc"><div class="mc-l">${zUiText('Com. bruta')}</div><div class="mc-v">${fmt(v.valor*v.pct)}</div><div class="mc-s">${(v.pct*100).toFixed(0)}%</div></div><div class="mc"><div class="mc-l">${zUiText('Com. lÃ­quida')}</div><div class="mc-v g">${fmt(comTotal(v))}</div></div><div class="mc"><div class="mc-l">${zUiText('Zelony')}</div><div class="mc-v">${fmt(comZ(v))}</div></div>`;
  }
  ...
  const rhRow=(['dir','fin','rh'].includes(role)&&v.pct_rh>0)?`<div class="eq-row" style="background:#EEF4FE;border-radius:6px;padding:6px 8px;margin-top:6px;border:1px solid #90B8E8;"><div style="width:25px;height:25px;border-radius:50%;background:#D0E4FF;border:1px solid #90B8E8;display:flex;align-items:center;justify-content:center;font-size:11px;flex-shrink:0;">${zUiText('🤝')}</div><div style="flex:1;min-width:0;"><div style="font-size:12px;font-weight:500;color:#1A56C4;">${zUiText('RH — Contratação')}</div><div style="font-size:9px;color:#4070B0;">${(v.pct_rh*100).toFixed(1)}%</div></div><span style="font-size:13px;font-weight:600;color:#1A56C4;">${fmt(comRH(v))}</span></div>`:'';
  const valorImposto=v.valor*v.pct*v.imp;
  const impostoRow=['dir','fin'].includes(role)?`<div style="margin-top:8px;border-top:1px dashed rgba(184,144,42,0.25);padding-top:8px;"><div class="eq-row" style="background:#FEF6EC;border-radius:6px;padding:6px 8px;margin:0 -2px;"><div style="width:25px;height:25px;border-radius:50%;background:#FEF0DC;border:1px solid #E8A040;display:flex;align-items:center;justify-content:center;font-size:11px;flex-shrink:0;">${zUiText('📋')}</div><div style="flex:1"><div style="font-size:12px;font-weight:500;color:#A05010;">${zUiText('Imposto sobre comissão')}</div><div style="font-size:9px;color:#B07030;">${(v.imp*100).toFixed(0)}% ${zUiText('sobre comissão bruta')} (${fmt(v.valor*v.pct)})</div></div><span style="font-size:13px;font-weight:600;color:#C06020;">- ${fmt(valorImposto)}</span></div></div>`:'';
...
function renderAnexosSec(v){
  ...
  if(!podeVer) return '';
  const tipos={comprovante:{label:'Comprovante',badge:'badge-comp',icon:'ðŸ§¾'},contrato:{label:'Contrato',badge:'badge-cont',icon:'ðŸ“„'},outro:{label:'Outro',badge:'badge-outro',icon:'ðŸ“Ž'}};
  const lista=v.anexos&&v.anexos.length?v.anexos.map((a,i)=>{
    ...
    const icon=['jpg','jpeg','png','gif','webp'].includes(ext)?'ðŸ–¼ï¸':ext==='pdf'?'ðŸ“•':'ðŸ“Ž';
    return`<div class="anexo-item"><span class="anexo-icon">${zUiText(icon)}</span><div class="anexo-info"><div class="anexo-nome" title="${zUiText(a.nome)}">${zUiText(a.nome)}</div><div class="anexo-meta">${zUiText(a.tamanho)} ${zUiText('Â·')} ${zUiText(a.data)} ${zUiText('Â·')} ${zUiText('por')} ${zUiText(a.por)}</div></div><span class="anexo-badge ${t.badge}">${zUiText(t.label)}</span><div class="anexo-btns"><button class="btn-anexo-view" onclick="verAnexo(${v.id},${i})">${zUiText('ðŸ‘ Ver')}</button>${podeEditar?`<button class="btn-anexo-del" onclick="delAnexo(${v.id},${i})">${zUiText('ðŸ—‘')}</button>`:''}</div></div>`;
  }).join(''):`<div style="text-align:center;padding:14px;font-size:11px;color:var(--tm);">${zUiText('Nenhum anexo ainda.')}</div>`;
  const uploadSection=podeEditar?`<div class="anexo-drop" id="anexo-drop-${v.id}" onclick="document.getElementById('anexo-input-${v.id}').click()"><div class="anexo-drop-icon">${zUiText('ðŸ“¤')}</div><div class="anexo-drop-txt"><strong>${zUiText('Clique para adicionar')}</strong> ${zUiText('ou arraste o arquivo aqui')}<br>${zUiText('PDF, JPG, PNG â€” mÃ¡x. 10MB')}</div></div><input type="file" class="anexo-input" id="anexo-input-${v.id}" accept=".pdf,.jpg,.jpeg,.png,.webp" multiple><div style="display:flex;gap:6px;flex-wrap:wrap;margin:-4px 0 2px;"><span style="font-size:10px;color:var(--tm);">${zUiText('Classificar como')}:</span><label style="display:flex;align-items:center;gap:4px;font-size:10px;color:var(--ts);cursor:pointer;"><input type="radio" name="tipo-anexo-${v.id}" value="comprovante" checked style="accent-color:var(--gold);"> ${zUiText('Comprovante')}</label><label style="display:flex;align-items:center;gap:4px;font-size:10px;color:var(--ts);cursor:pointer;"><input type="radio" name="tipo-anexo-${v.id}" value="contrato" style="accent-color:var(--gold);"> ${zUiText('Contrato')}</label><label style="display:flex;align-items:center;gap:4px;font-size:10px;color:var(--ts);cursor:pointer;"><input type="radio" name="tipo-anexo-${v.id}" value="outro" style="accent-color:var(--gold);"> ${zUiText('Outro')}</label></div>`:'';
  return`<div class="anexos-sec"><div class="anexos-h"><span>${zUiText('ðŸ“Ž Anexos')} <span style="font-weight:400;color:var(--tm);text-transform:none;letter-spacing:0;">(${v.anexos?v.anexos.length:0} ${zUiText(`arquivo${v.anexos&&v.anexos.length!==1?'s':''}`)})</span></span></div><div class="anexos-body">${uploadSection}<div class="anexo-list">${lista}</div></div></div>`;
}
...
function processAnexoFiles(files,vendaId){
  ...
  Array.from(files).forEach(file=>{
    if(file.size>10*1024*1024){showToast(zUiText('âš ï¸'),zUiText(`"${file.name}" Ã© maior que 10MB.`));processados++;return;}
    ...
    reader.onload=e=>{
      ...
      processados++;
      if(processados===total){dbAtualizarVenda(v).catch(err=>console.error(err));salvarLS();showVDetail(vendaId);showToast(zUiText('âœ…'),zUiText(`${total} arquivo${total>1?'s':''} adicionado${total>1?'s':''}!`));}
    };
    reader.readAsDataURL(file);
  });
}
function delAnexo(vendaId,idx){const v=VENDAS.find(x=>x.id===vendaId);if(!v||!v.anexos)return;if(!confirm(zUiText(`Remover "${v.anexos[idx].nome}"?`)))return;v.anexos.splice(idx,1);dbAtualizarVenda(v).catch(err=>console.error(err));salvarLS();showVDetail(vendaId);showToast(zUiText('ðŸ—‘'),zUiText('Anexo removido.'));}
function verAnexo(vendaId,idx){const v=VENDAS.find(x=>x.id===vendaId);if(!v||!v.anexos||!v.anexos[idx])return;const a=v.anexos[idx];document.getElementById('av-nome').textContent=zUiText(a.nome);const cont=document.getElementById('av-content');if(a.mime&&a.mime.startsWith('image/')){cont.innerHTML=`<img src="${a.dataUrl}" alt="${zUiText(a.nome)}">`;}else if(a.mime==='application/pdf'){cont.innerHTML=`<iframe src="${a.dataUrl}" title="${zUiText(a.nome)}"></iframe>`;}else{cont.innerHTML=`<div style="color:#fff;font-size:13px;padding:20px;background:rgba(255,255,255,0.1);border-radius:8px;">${zUiText('PrÃ©-visualizaÃ§Ã£o nÃ£o disponÃ­vel.')}<br><a href="${a.dataUrl}" download="${zUiText(a.nome)}" style="color:var(--gold-l);margin-top:10px;display:inline-block;">${zUiText('â¬‡ Baixar arquivo')}</a></div>`;}document.getElementById('anexo-viewer').classList.add('show');}
...
function renderRel(){
  ...
  const comBruta=ativas.reduce((s,v)=>s+(v.valor*v.pct),0);
  ...
  document.getElementById('rel-mets').innerHTML=`
    ...
    ${isAdmin?`<div class="mc"><div class="mc-l">${zUiText('Com. Bruta')}</div><div class="mc-v g">${fmtK(comBruta)}</div><div class="mc-s">${zUiText('antes do imposto')}</div></div>`:''}
  ...
  document.getElementById('rel-body').innerHTML=l.map(v=>`<tr style="${v.distratada?'opacity:0.5;text-decoration:line-through;':''}"><td>${zUiText(v.data)}</td><td><span class="zbg bg-b" style="font-size:8px;">${zUiText(v.mes)}</span></td><td style="max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${zUiText(v.cliente.split('/')[0].trim())}</td><td>${zUiText(v.produto)}</td><td>${zUiText(v.construtora)}</td><td><span style="font-size:9px;background:var(--bg2);border:1px solid var(--bd);border-radius:3px;padding:1px 5px;">${zUiText(v.origem||'â€”')}</span></td><td>${zUiText(v.corretor)}</td><td>${zUiText(v.gerente||'â€”')}</td>${isAdmin?`<td style="font-size:9px;">${zUiText(v.cca||'â€”')}</td>`:''}<td class="pos">${fmt(v.valor)}</td>${isAdmin?`<td class="pos">${fmt(v.valor*v.pct)}</td><td class="pos">${fmt(comTotal(v))}</td><td class="pos" style="color:var(--gold);">${fmt(comZ(v))}</td>`:''} ${!isAdmin?`<td class="pos">${fmt(comVis(v))}</td>`:''}${totalBonus>0&&isAdmin?`<td class="pos" style="color:#2E9E6E;">${v.bonus>0?fmt(v.bonus):zUiText('â€”')}</td>`:''}<td><span class="spill${v.etapa===ETAPAS.length-1?' f':''}">${zUiText(ETAPAS[v.etapa])}</span>${v.distratada?`<span style="font-size:8px;color:#C05030;margin-left:3px;">${zUiText('âš ï¸')}</span>`:''}</td><td style="font-size:9px;">${zUiText(v.unidade||'â€”')}</td></tr>`).join('');
  ...
  if(ativas.length>0){
    ...
    div.innerHTML=`<div style="background:var(--bg);border:1px solid var(--bd);border-radius:9px;padding:12px 14px;"><div style="font-size:9px;text-transform:uppercase;letter-spacing:0.08em;color:var(--tm);font-weight:600;margin-bottom:8px;">${zUiText('ðŸ“Œ Origem dos leads')}</div>${origemHTML||`<div style="font-size:11px;color:var(--tm);">${zUiText('Sem dados')}</div>`}</div><div style="background:var(--bg);border:1px solid var(--bd);border-radius:9px;padding:12px 14px;"><div style="font-size:9px;text-transform:uppercase;letter-spacing:0.08em;color:var(--tm);font-weight:600;margin-bottom:8px;">${zUiText('ðŸ—ï¸ Top construtoras (VGV)')}</div>${constHTML||`<div style="font-size:11px;color:var(--tm);">${zUiText('Sem dados')}</div>`}</div>`;
    relMets.after(div);
  }
}
...
function exportXLSX(){
  ...
  setTimeout(()=>{
    ...
    const porConst={};l.filter(v=>!v.distratada).forEach(v=>{const c=v.construtora||'Ã¢â‚¬â€';if(!porConst[c])porConst[c]={vendas:0,vgv:0,comBruta:0,comLiq:0};porConst[c].vendas++;porConst[c].vgv+=v.valor;porConst[c].comBruta+=v.valor*v.pct;porConst[c].comLiq+=comTotal(v);});
    ...
    showToast(zUiText('âœ…'),zUiText('Excel exportado com 3 abas!'));
    btn.classList.remove('loading');btn.textContent=zUiText('â¬‡ Excel');
  },300);
}
...
function exportPDF(){
  ...
  setTimeout(()=>{
    try{
      ...
      const vgv=ativas.reduce((s,v)=>s+v.valor,0),comBruta=ativas.reduce((s,v)=>s+(v.valor*v.pct),0),comLiq=ativas.reduce((s,v)=>s+comTotal(v),0),zelony=ativas.reduce((s,v)=>s+comZ(v),0),conc=ativas.filter(v=>v.etapa===ETAPAS.length-1).length;
      const kpis=[['VGV TOTAL',fmt(vgv)],[zUiText('COM. BRUTA'),fmt(comBruta)],[zUiText('COM. LÃQUIDA'),fmt(comLiq)],['ZELONY',fmt(zelony)],[zUiText('VENDAS'),String(ativas.length)],[zUiText('CONCLUÃDAS'),String(conc)]];
      ...
      showToast(zUiText('âœ…'),zUiText('PDF exportado com anÃ¡lises!'));
    }catch(e){console.error(e);showToast(zUiText('âŒ'),zUiText('Erro ao gerar PDF.'));}
    btn.classList.remove('loading');btn.textContent=zUiText('â¬‡ PDF');
  },300);
}
