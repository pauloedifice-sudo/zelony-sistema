// CARTEIRA - parte 4/4: linhas de tabela, cards mobile, render principal e registro do modulo
...
function renderCarteira() {
  ...
  document.getElementById('carteira-content').innerHTML = `
    ...
    ${perdido > 0 ? `<div style="background:rgba(185,74,58,0.14);border:1px solid rgba(185,74,58,0.34);border-radius:9px;padding:12px 16px;display:flex;align-items:center;justify-content:space-between;gap:10px;"><div><div style="font-size:9px;text-transform:uppercase;letter-spacing:0.08em;color:#D88F83;font-weight:700;margin-bottom:2px;">${zUiText(`⚠ Perdido com distrato${distratas.length > 1 ? 's' : ''}`)}</div><div style="font-size:11px;color:#D88F83;opacity:0.8;">${zUiText(`${distratas.length} venda${distratas.length > 1 ? 's' : ''} distratada${distratas.length > 1 ? 's' : ''}`)}</div></div><div style="font-size:22px;font-weight:700;color:#D88F83;font-family:'Playfair Display',serif;">- ${fmtCarteiraValor(perdido)}</div></div>` : ''}
