...
function showVDetail(id){
  ...
  let mH='';
  if(role==='cor'){
    mH=`<div class="mc a"><div class="mc-l">${zUiText('Valor de venda')}</div><div class="mc-v g">${fmt(v.valor)}</div></div><div class="mc"><div class="mc-l">${zUiText('Minha comissão')}</div><div class="mc-v g">${fmt(comC(v))}</div><div class="mc-s">${(v.pct_cor*100).toFixed(1)}% ${zUiText('líq.')}</div></div><div class="mc"><div class="mc-l">${zUiText('Etapa')}</div><div class="mc-v">${v.etapa+1}/${ETAPAS.length}</div></div><div class="mc"><div class="mc-l">${zUiText('Status')}</div><div class="mc-v" style="font-size:13px;">${isFinal?zUiText('✓ Pago'):zUiText('Andamento')}</div></div>`;
  }else if(role==='cap'){
    mH=`<div class="mc a"><div class="mc-l">${zUiText('Valor venda')}</div><div class="mc-v g">${fmt(v.valor)}</div></div><div class="mc"><div class="mc-l">${zUiText('Com. corretor')}</div><div class="mc-v">${fmt(comC(v))}</div></div><div class="mc"><div class="mc-l">${zUiText('Com. capitão')}</div><div class="mc-v g">${fmt(comCap(v))}</div></div><div class="mc"><div class="mc-l">${zUiText('Total visível')}</div><div class="mc-v g">${fmt(comCap(v)+comC(v))}</div></div>`;
  }else if(role==='ger'){
    mH=`<div class="mc a"><div class="mc-l">${zUiText('Valor venda')}</div><div class="mc-v g">${fmt(v.valor)}</div></div><div class="mc"><div class="mc-l">${zUiText('Com. corretor')}</div><div class="mc-v">${fmt(comC(v))}</div></div><div class="mc"><div class="mc-l">${zUiText('Com. capitão')}</div><div class="mc-v">${fmt(comCap(v))}</div></div><div class="mc"><div class="mc-l">${zUiText('Com. gerente')}</div><div class="mc-v g">${fmt(comG(v))}</div></div>`;
  }else{
    mH=`<div class="mc a"><div class="mc-l">${zUiText('Valor venda')}</div><div class="mc-v g">${fmt(v.valor)}</div></div><div class="mc"><div class="mc-l">${zUiText('Com. bruta')}</div><div class="mc-v">${fmt(v.valor*v.pct)}</div><div class="mc-s">${(v.pct*100).toFixed(0)}%</div></div><div class="mc"><div class="mc-l">${zUiText('Com. líquida')}</div><div class="mc-v g">${fmt(comTotal(v))}</div></div><div class="mc"><div class="mc-l">${zUiText('Zelony')}</div><div class="mc-v">${fmt(comZ(v))}</div></div>`;
  }

  const mbs=[];
  mbs.push({n:v.corretor,c:'Corretor',val:comC(v),pct:v.pct_cor});
  if(['cap','ger','dir','fin','dono'].includes(role)&&v.capitao) mbs.push({n:v.capitao,c:'CapitÃ£o',val:comCap(v),pct:v.pct_cap});
  ...
  if(['fin','dono'].includes(role)) mbs.push({n:'Zelony',c:'ImobiliÃ¡ria',val:comZ(v),pct:Math.max(v.pct-v.pct_cor-v.pct_cap-v.pct_ger-v.pct_dir-(v.pct_dir2||0),0)});
  ...
  const distH=`<div class="sec"><div class="sec-h">${zUiText('Distribuição da comissão')}</div><div class="sec-b">${mbs.map(m=>{
    ...
    return`<div class="eq-row" style="flex-wrap:wrap;"><div class="eav">${ini(m.n)}</div><div style="flex:1;min-width:0;"><div style="font-size:12px;font-weight:500;">${zUiText(m.n)}</div><div style="font-size:9px;color:var(--tm);">${zUiText(m.c)} ${zUiText('·')} ${(m.pct*100).toFixed(2)}%</div>${bancoPix}</div><span style="font-size:13px;font-weight:600;color:var(--gold);white-space:nowrap;margin-left:8px;">${fmt(m.val)}</span></div>`;
  ...
  <div class="prog-wrap">
    <div style="font-size:9px;text-transform:uppercase;letter-spacing:0.1em;color:var(--gold);font-weight:600;margin-bottom:10px;">${zUiText('Evolução da venda')}</div>
    ...
    ${!['fin','dir','dono'].includes(role)?`<div style="font-size:9px;color:var(--tm);margin-top:5px;">${zUiText('Apenas Financeiro ou Diretor podem avançar etapas.')}</div>`:''}
  </div>
  ${distH}${bonusH}
  <div class="sec"><div class="sec-h">${zUiText('Histórico')}</div><div class="sec-b">${hH}</div></div>
  ${anexosH}`;
...
function renderRel(){
  ...
  const porConst={};ativas.forEach(v=>{const c=v.construtora||'â€”';if(!porConst[c])porConst[c]={n:0,vgv:0};porConst[c].n++;porConst[c].vgv+=v.valor;});
  ...
  document.getElementById('rel-mets').innerHTML=`
    ...
    ${isAdmin?`<div class="mc"><div class="mc-l">${zUiText('Com. Líquida')}</div><div class="mc-v g">${fmtK(comLiq)}</div><div class="mc-s">${zUiText('após imposto')}</div></div>`:''}
    ${isAdmin?`<div class="mc"><div class="mc-l">${zUiText('Zelony')}</div><div class="mc-v g">${fmtK(comZel)}</div><div class="mc-s">${zUiText('lucro líquido')}</div></div>`:''}
    ${!isAdmin?`<div class="mc"><div class="mc-l">${zUiText(lblCom())}</div><div class="mc-v g">${fmtK(mc)}</div><div class="mc-s">${zUiText('total período')}</div></div>`:''}
    <div class="mc"><div class="mc-l">${zUiText('Concluídas')}</div><div class="mc-v" style="color:#2E9E6E;">${conc.length}</div><div class="mc-s">${zUiText('comissão recebida')}</div></div>
    ...
    ${totalBonus>0&&isAdmin?`<div class="mc"><div class="mc-l">${zUiText('Bônus Total')}</div><div class="mc-v g">${fmtK(totalBonus)}</div></div>`:''}`;
  ...
  document.getElementById('rel-thead').innerHTML=`<th>${zUiText('Data')}</th><th>${zUiText('Mês')}</th><th>${zUiText('Cliente')}</th><th>${zUiText('Produto')}</th><th>${zUiText('Construtora')}</th><th>${zUiText('Origem')}</th><th>${zUiText('Corretor')}</th><th>${zUiText('Gerente')}</th>${isAdmin?`<th>${zUiText('CCA')}</th>`:''}<th>${zUiText('Valor')}</th>${isAdmin?`<th>${zUiText('Com. Bruta')}</th><th>${zUiText('Com. Líquida')}</th><th>${zUiText('Zelony')}</th>`:''}${!isAdmin?`<th>${zUiText(lblCom())}</th>`:''} ${totalBonus>0&&isAdmin?`<th>${zUiText('Bônus')}</th>`:''}<th>${zUiText('Etapa')}</th><th>${zUiText('Unidade')}</th>`;
...
function exportXLSX(){
  ...
  setTimeout(()=>{
    let l=vendasU(relMes==='TODOS'?VENDAS:VENDAS.filter(v=>v.mes===relMes));
    const isAdmin=['dir','fin','dono'].includes(role);
    const cabecalho=[zUiText('Data'),zUiText('Mês'),zUiText('Cliente'),zUiText('Produto'),zUiText('Construtora'),zUiText('Origem'),zUiText('Corretor'),zUiText('Gerente'),'CCA','Valor (R$)',zUiText('Com. Bruta (R$)'),zUiText('Com. Líquida (R$)'),'Zelony (R$)',zUiText('Bônus (R$)'),zUiText('Etapa'),zUiText('Unidade'),zUiText('Status')];
    ...
    const porConst={};l.filter(v=>!v.distratada).forEach(v=>{const c=v.construtora||'â€”';if(!porConst[c])porConst[c]={vendas:0,vgv:0,comBruta:0,comLiq:0};porConst[c].vendas++;porConst[c].vgv+=v.valor;porConst[c].comBruta+=v.valor*v.pct;porConst[c].comLiq+=comTotal(v);});
    const wsConst=XLSX.utils.aoa_to_sheet([[zUiText('Construtora'),zUiText('Vendas'),'VGV (R$)',zUiText('Com. Bruta (R$)'),zUiText('Com. Líquida (R$)')],...Object.entries(porConst).sort((a,b)=>b[1].vgv-a[1].vgv).map(([c,d])=>[zUiText(c),d.vendas,Math.round(d.vgv),Math.round(d.comBruta),Math.round(d.comLiq)])]);
...
function exportPDF(){
  ...
  setTimeout(()=>{
    try{
      ...
      doc.setFont('helvetica','bold');doc.setFontSize(16);doc.setTextColor(255,255,255);doc.text(zUiText('ZELONY IMÓVEIS'),W/2,12,{align:'center'});
      doc.setFont('helvetica','normal');doc.setFontSize(8);doc.setTextColor(255,248,220);doc.text(zUiText(`Relatório ${relMes} · Gerado em ${hoje()}`),W/2,20,{align:'center'});
      ...
      const kpis=[['VGV TOTAL',fmt(vgv)],[zUiText('COM. BRUTA'),fmt(comBruta)],[zUiText('COM. LÍQUIDA'),fmt(comLiq)],['ZELONY',fmt(zelony)],[zUiText('VENDAS'),String(ativas.length)],[zUiText('CONCLUÍDAS'),String(conc)]];
      ...
      doc.autoTable({startY:49,head:[[zUiText('Data'),zUiText('Mês'),zUiText('Cliente'),zUiText('Produto'),zUiText('Construtora'),zUiText('Origem'),zUiText('Corretor'),zUiText('Gerente'),'CCA',zUiText('Valor'),'Com.Bruta',zUiText('Com.Líq.'),'Zelony',zUiText('Bônus'),zUiText('Etapa'),'Unid.']],body:l.map(v=>[zUiText(v.data),zUiText(v.mes),zUiText(v.cliente.split('/')[0].trim().slice(0,18)),zUiText(v.produto.slice(0,12)),zUiText(v.construtora.slice(0,10)),zUiText(v.origem||''),zUiText(v.corretor.split(' ')[0]),zUiText((v.gerente||'').split(' ')[0]),zUiText(v.cca||'—'),fmt(v.valor),fmt(v.valor*v.pct),fmt(comTotal(v)),fmt(comZ(v)),v.bonus>0?fmt(v.bonus):zUiText('—'),zUiText(ETAPAS[v.etapa].slice(0,14))+(v.distratada?' '+zUiText('⚠️'):''),zUiText(v.unidade||'—')]),theme:'plain',headStyles:{fillColor:[253,248,238],textColor:[184,144,42],fontSize:6,fontStyle:'bold',halign:'center',lineColor:[220,185,100],lineWidth:0.3},bodyStyles:{fontSize:6,textColor:[60,48,30]},alternateRowStyles:{fillColor:[250,245,236]},columnStyles:{0:{cellWidth:10},1:{cellWidth:12},2:{cellWidth:25},3:{cellWidth:16},4:{cellWidth:16},5:{cellWidth:14},6:{cellWidth:16},7:{cellWidth:16},8:{cellWidth:12},9:{cellWidth:17},10:{cellWidth:16},11:{cellWidth:16},12:{cellWidth:14},13:{cellWidth:12},14:{cellWidth:20},15:{cellWidth:10}},margin:{left:7,right:7},didDrawPage:(d)=>{doc.setFont('helvetica','normal');doc.setFontSize(5.5);doc.setTextColor(140,110,60);doc.text(zUiText(`Zelony Imóveis · ${hoje()} · Pág. ${d.pageNumber}`),W/2,doc.internal.pageSize.getHeight()-4,{align:'center'});}});
      doc.addPage();doc.setFillColor(184,144,42);doc.rect(0,0,W,14,'F');doc.setFont('helvetica','bold');doc.setFontSize(11);doc.setTextColor(255,255,255);doc.text(zUiText('Análise do Período'),W/2,9,{align:'center'});
      const pC={};ativas.forEach(v=>{const c=v.construtora||'â€”';if(!pC[c])pC[c]={n:0,vgv:0,com:0};pC[c].n++;pC[c].vgv+=v.valor;pC[c].com+=comZ(v);});
